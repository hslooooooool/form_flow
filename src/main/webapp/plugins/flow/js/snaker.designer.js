	...
	designer.config={
		...
		path:{
			...
			props:{
				...
				name:{name:"name",label:"名称",tabs:{id:'base-prop',name:'基本属性'},value:"",editor:function(){return new designer.editors.inputReadonly()}},
				displayName:{name:"displayName",label:"显示",tabs:{id:'base-prop',name:'属性'},value:"",editor:function(){return new designer.editors.textEditor()}},
                type:{name:"type",label:"类型",tabs:{id:'base-prop',name:'属性'},value:"",editor:function(){return new designer.editors.selectEditor("dict/item/FLOW_LINE_TYPE.json");}},
                isCheckForm:{name:"isCheckForm",label:"验证表单",tabs:{id:'base-prop',name:'属性'},value:"", editor:function(){return new designer.editors.selectEditor("dict/item/YES_OR_NO.json",utils.YES_OR_NO.YES);}},
                sortNum:{name:"sortNum",label:"显示序号",tabs:{id:'base-prop',name:'属性'},value:"", editor:function(){return new designer.editors.inputEditor()}}
				// expr:{name:"expr",label:"表达式",tabs:{id:'base-prop',name:'基本属性'},value:"",editor:function(){return new designer.editors.inputEditor()}}
			}
		},
		tools:{
			attr:{
		...
		if(designer.config.editable){
			...
			snaker(c).droppable({
				...
				drop:function(c,i){
					var offsetLeft = 0;
					var offsetTop = 0;
					if(!utils.isEmpty($("#menu-nav").attr("id")) && $("#menu-nav").css("display") != 'none') {
						offsetLeft += $("#menu-nav").width();
					}
					/*if(!utils.isEmpty($("#toolbox").attr("id"))) {
						offsetLeft += $("#toolbox").width();
					}*/
					if(!utils.isEmpty($(".header").attr("class"))) {
						offsetTop += $(".header").height();
					}
					if(!utils.isEmpty($(".header-bottom").attr("class"))) {
						offsetTop += $(".header-bottom").height();
					}
